...
在你的佈局中，你可以在客戶端使用 `useEffect` 或者如 [SWR](https://swr.vercel.app/) 的函式庫獲取資料。由於這個檔案不是一個[頁面](/docs/basic-features/pages.md)，因此目前不能使用 `getStaticProps` 或 `getServerSideProps`。
